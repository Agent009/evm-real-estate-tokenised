...
const proxyModule = buildModule("ProxyModule", m => {
  ...
  const encodedFunctionCall = m.encodeFunctionCall(property, "initialize", [proxyAdminOwner, proxyAdminOwner]);
  const proxy = m.contract("TransparentUpgradeableProxy", [property, proxyAdminOwner, encodedFunctionCall]);
 ...
 * It creates a contract instance for the Property contract using the proxy from the previous module.
 */
const propertyModule = buildModule("PropertyModule", m => {
  ...
  const instance = m.contractAt("Property", proxy);

  // Return the contract instance, along with the original proxy and proxyAdmin contracts
  // so that they can be used by other modules, or in tests and scripts.
  return { instance, proxy, proxyAdmin };
});
